...
def parse_args():
    ...
    parser.add_argument('--out_dir', type=str, default='./results_with_replacement', help='output directory for the results')
    return parser.parse_args()

def parse_pretrain_dirname(dirname, preargnamelist):
    arglist = dirname.split('_')
    
    if (len(arglist) != len(preargnamelist)):
        print('Error in pretrain dir name', dirname)
        print('Expected', len(preargnamelist), 'but got', len(arglist))
        print('Expected', preargnamelist, 'but got', arglist)
        exit(0)
...
if __name__ == '__main__':
    ...
    for dirname in wrapped_outer:
        ...
        for undirname in wrapped_inner:
            if ("(replacement)" not in undirname and "pretrainmodel" not in undirname):
                continue
            ...
            if row is None:
                ...
                row["id"] = dirname
                if row["unlearn_method"] == "Naive" and row["exp_name"] == "unlearn(replacement)":
    ...
    for key in wrapped_keys:
        ...
        for unlearn_id in wrapped_unlearn_ids:
            ...
            if unlearn_method in accumulated_results[key]:
                ...
                if (new_run_weighted > existing_run_weighted and len(rows_for_unlearn_id) == accumulated_results[key][unlearn_method]["len"]) or len(rows_for_unlearn_id) > accumulated_results[key][unlearn_method]["len"]:
        ...
        plt.savefig(os.path.join(args.out_dir, f"{key}_test_retain_acc.pdf"), dpi=300, format="pdf", bbox_inches='tight')
        plt.close(fig)
        ...
        plt.tight_layout()
        plt.legend(loc='upper left')
        plt.savefig(os.path.join(args.out_dir, f"{key}_corr_acc.pdf"), dpi=300, format="pdf", bbox_inches='tight')
        plt.close(fig)
    ...
    for key in accumulated_results:
        ...
        # print(accumulated_results[key].keys())
        # exit(0)

...
if __name__ == '__main__':
    ...
    if opt.dataset == "CIFAR10":
        ...
        opt.num_classes = 2
    elif opt.dataset == "LFWPeople":
        opt.num_classes = 5749
    elif opt.dataset == "CelebA":
        opt.num_classes = 10177
    elif opt.dataset == "DermNet":
        opt.num_classes = 23
    elif opt.dataset == "Pneumonia":
        opt.num_classes = 2

    if opt.dataset_method == 'poison':
        opt.dataset_method = 'poisoning'
    ...
    if opt.dataset_method == 'poisoning':
        ...
        eval_loaders['correct'] = adversarial_test_loader
    else:
    ...
    if opt.dataset_method == 'interclasslabelswap':
        ...
        eval_loaders['correct'] = torch.utils.data.DataLoader(test_set, batch_size=opt.batch_size, shuffle=False, sampler=SubsetSequentialSampler(indices), num_workers=4, pin_memory=True)
    ...
    if not exists(opt.pretrain_file_prefix + '/Naive_pretrainmodel/model.pth'):
        ...
        method.compute_and_save_results(train_test_loader, test_loader, adversarial_train_loader, adversarial_test_loader, wandb_run=wandb_run, method_handle="base", save_model=True)
    ...
    if wandb_run is not None and opt.unlearn_method == 'Reload' and corr_score is not None:
        wandb_run.log({
            f"{opt.unlearn_method}_val_top1": outcome + corr_score,
        })
        wandb_run.finish()
